...
class QuantileNetworkModule(nn.Module):
    def __init__(self, X_means, X_stds, y_mean, y_std, n_out):
        ...
        self.fc_in = nn.Sequential(
                ...
                nn.ReLU(),
                #nn.BatchNorm1d(200),
                nn.Linear(64, self.n_out if len(self.y_mean.shape) == 1 else self.n_out * self.y_mean.shape[1]))
...
class QuantileNetwork:
    ...
    def fit(self, X, y, batch_size, nepochs):
        self.model = fit_quantiles(X, y, quantiles=self.quantiles, lossfn=self.lossfn,batch_size=batch_size,nepochs=nepochs)
...
def fit_quantiles(X, y, quantiles=0.5, lossfn = 'marginal',
                    ...
                    init_model=None, splits=None, file_checkpoints=False,
