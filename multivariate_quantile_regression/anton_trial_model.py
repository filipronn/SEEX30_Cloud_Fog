...
class QuantileNetworkMM(nn.Module):
    def __init__(self,n_in,n_out,y_dim, X_means, X_stds, y_mean, y_std, seq, data_norm):
    ...
    def predict(self,x):
        ...
        self.zero_grad()
        if self.data_norm:
            tX=torch.FloatTensor((x - self.X_means) / self.X_stds)
...
class QuantileNetwork():
    ...
    def fit(self, X, y, train_indices, validation_indices, batch_size, nepochs, sequence=None,lr=0.001,data_norm=False):
        self.model = fit_quantiles(X, y, train_indices, validation_indices, quantiles=self.quantiles, batch_size=batch_size, sequence=sequence, n_epochs=nepochs, lr=lr, data_norm=data_norm)
...
def fit_quantiles(X,y,train_indices,validation_indices,quantiles,n_epochs,batch_size,sequence,lr,data_norm,loss='quantile',file_checkpoints=True):
    ...
    if data_norm:
        tX = torch.FloatTensor((X - X_mean) / X_std)
        tY = torch.FloatTensor((y - y_mean) / y_std)
    else:
        tX = torch.FloatTensor(X)
    ...
    model=QuantileNetworkMM(n_in,n_out,y_dim,X_mean,X_std,y_mean,y_std,seq=sequence,data_norm=data_norm)

    optimizer = optim.AdamW(model.parameters(),lr = lr) #Set optimiser, atm Stochastic Gradient Descent
