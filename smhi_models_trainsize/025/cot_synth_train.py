...
# Global vars
BASE_PATH_DATA = 'cot_train/data/synthetic-cot-data_025'
...
copyfile("cot_train/cot_synth_train.py", os.path.join(log_dir, "cot_synth_train.py"))
...
# Read data
print("Reading data")
trainset = np.load(os.path.join(BASE_PATH_DATA, 'trainset_smhi.npy'),allow_pickle=True)
valset = np.load(os.path.join(BASE_PATH_DATA, 'valset_smhi.npy'),allow_pickle=True)
...
print(device)
inputs_train = torch.Tensor(inputs_train.astype(float)).to(device)
inputs_val = torch.Tensor(inputs_val.astype(float)).to(device)
gts_train = torch.Tensor(gts_train.astype(float)).to(device)
gts_val = torch.Tensor(gts_val.astype(float)).to(device)
